    ...
    def update_koneksi(self):
        if self.tableView.currentIndex().row() > -1:
            if str(self.btnUpdate.text()) == "Edit":
                ...
                sw = str(self.inpStatusWarga.currentText())
                query = QSqlQuery()
                query.prepare("UPDATE penduduk SET Tanggal_Pembuatan=:pembuatan ,Nama_Lengkap=:nama, Alamat=:alamat, RT=:rt, Agama=:agama, Pendidikan=:pendidikan, Status_Nikah=:sn,Pekerjaan=:pekerjaan, Status_Warga=:sw WHERE  NIK = :nik")
